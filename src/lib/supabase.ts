...
export async function getRepoByUrl(url: string) {
	const { data, error } = await supabase.from('repositories').select('*').eq('url', url).single();
...
export async function insertClassCounts(repoId: number, classCounts: Record<string, number>) {
	...
	const { data, error } = await supabase.from('tailwind_classes').insert(rows);
...
export async function getClassCountsForRepo(repoId: number) {
	const { data, error } = await supabase.from('tailwind_classes').select('*').eq('repo_id', repoId);
...
export async function getTopClassesGlobal(limit: number = 50) {
	const { data, error } = await supabase.rpc('get_top_classes_global', { limit_count: limit });
