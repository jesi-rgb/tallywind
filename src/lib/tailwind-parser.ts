...
const CLASSNAME_FUNCTION_REGEX = /(?:tw|cls|clsx|classnames|cva)\((?:["']\s*([^"']*)\s*["']|{(?:[^{}]|{(?:[^{}]|{[^{}]*})*})*}|(?:[^()]|{(?:[^{}]|{[^{}]*})*})*)\)/g;
...
export function countTailwindClasses(files: { name: string, content: string }[]): Record<string, number> {
